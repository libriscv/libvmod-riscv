...
{
	if (e == vmod_enum_ON_REQUEST) return (vcall_info){1, HDR_REQ, HDR_INVALID};
	...
	if (e == vmod_enum_ON_DELIVER) return (vcall_info){7, HDR_REQ, HDR_RESP};
...
VCL_INT vmod_run(VRT_CTX)
{
	...
	VCL_ENUM e = vmod_enum_ON_REQUEST;
	switch (ctx->method) {
	case VCL_MET_RECV:
		e = vmod_enum_ON_REQUEST;
		break;
	case VCL_MET_HASH:
		e = vmod_enum_ON_HASH;
		break;
	case VCL_MET_SYNTH:
		e = vmod_enum_ON_SYNTH;
		break;
	case VCL_MET_BACKEND_FETCH:
		e = vmod_enum_ON_BACKEND_FETCH;
		break;
	case VCL_MET_BACKEND_RESPONSE:
		e = vmod_enum_ON_BACKEND_RESPONSE;
		break;
	case VCL_MET_BACKEND_ERROR:
		e = vmod_enum_ON_BACKEND_ERROR;
		break;
	case VCL_MET_DELIVER:
		e = vmod_enum_ON_DELIVER;
		break;
	default:
		/* Unsupported method */
		if (ctx && ctx->vsl) {
