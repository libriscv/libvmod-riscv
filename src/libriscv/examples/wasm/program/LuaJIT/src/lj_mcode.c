...
#else
  register uintptr_t flush_start __asm__("a0") = (uintptr_t)start;
  register uintptr_t flush_end __asm__("a1") = (uintptr_t)end;
  register long syscall_id __asm__("a7") = __NR_riscv_flush_icache;
  __asm__ volatile("ecall" : "+r"(flush_start) : "r"(flush_end), "r"(syscall_id));
#endif
