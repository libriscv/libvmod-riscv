...
#elif LJ_TARGET_POSIX || 1

#if LJ_TARGET_LINUX || 1

  #define SYS_getrandom 278 /* 64-bit RISC-V */
  register void* rs_u __asm__("a0") = rs->u;
  register long rs_result __asm__("a0");
  register long rs_len __asm__("a1") = sizeof(rs->u);
  register long syscall_id __asm__("a7") = SYS_getrandom;
  __asm__("ecall" : "=m"(*(char *)rs_u), "=r"(rs_result) : "r"(rs_u), "r"(rs_len), "r"(syscall_id));
  goto ok;
