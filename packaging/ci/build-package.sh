#!/usr/bin/env bash
#
# Build a native libvmod-riscv package (.deb or .rpm) for Varnish Enterprise,
# matching the enterprise package naming used by e.g. libvmod-sledge:
#
#   libvmod-riscv_1.0~6.0.18r2-1~noble_amd64.deb
#   libvmod-riscv-1.0~6.0.18r2-1.el9.x86_64.rpm
#
...
build_deb() {
	...
	local vcp_full vcp_nodist vcp_up vcp_rel rv srcver
	...
	rv=$(riscv_version)
	srcver="${rv}~${vcp_up}"
	log "Versions: riscv=$rv varnish-plus=$vcp_full -> ${srcver}-${vcp_rel}~${PKG_CODENAME}"
...
build_rpm() {
	...
	# compiler. el8's gcc 8 is too old, so use gcc-toolset-13 there (base gcc-c++
	# is still installed to satisfy the spec's BuildRequires).
	case "${PKG_DIST:-}" in
		el8)
	...
	local vcp_up vcp_rel dist rv srcver
	...
	rv=$(riscv_version)
	srcver="${rv}~${vcp_up}"
	log "Versions: riscv=$rv varnish-plus=${vcp_up}-${vcp_rel}${dist} -> ${srcver}-${vcp_rel}${dist}"

	# rpmbuild needs a source tarball that already contains ./configure, so
	# bootstrap and use the automake `make dist' tarball (top dir libvmod-riscv-<rv>).
	log "Generating source tarball via make dist"
	./autogen.sh
	./configure
	make dist
	local srcname="libvmod-riscv-${rv}"
	...
	rpmbuild -bb \
		...
		packaging/redhat/libvmod-riscv.spec
...
# rst2man (from docutils) is required to build the man page. It is not always
